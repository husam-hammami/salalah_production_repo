...
def get_fcl_full():
    """
    Get all FCL archive records.
    Optional query parameter: line_running (true/false) to filter by line status.
    ...
    with get_db_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            if line_running_filter is not None:
                ...
                # Return all records if no filter specified
                cursor.execute("""
                    SELECT * FROM fcl_monitor_logs_archive
                    ORDER BY created_at ASC
                """)
...
def get_mila_archive_summary():
    ...
    try:
        ...
        order_name = request.args.get('order_name')  # ✅ Get order name filter
        ...
        # ✅ Convert UTC to Dubai timezone (database stores Dubai time as naive datetime)
        ...
        # ✅ MILA-SPECIFIC: Shift time range by -1 hour ONLY for daily reports (>= 24 hours)
        # For hourly/multi-hour reports (< 24 hours), use exact time (no shift)
        # Daily: User selects 5 AM to 5 AM -> Query 4 AM to 4 AM directly
        # Hourly: User selects 7 AM to 8 AM -> Query 7 AM to 8 AM directly (no shift)
        # This is ONLY for MILA reports, not for FCL/SCL/other reports
        time_diff = end_dubai - start_dubai
        is_daily_or_longer = time_diff >= timedelta(hours=23, minutes=59)  # >= 24 hours (daily/weekly/monthly)
        
        if is_daily_or_longer:
            # For daily/weekly/monthly reports, apply -1 hour shift
            start_dubai = start_dubai - timedelta(hours=1)
            end_dubai = end_dubai - timedelta(hours=1)
            logger.info(f"📊 [MIL-A Summary] Daily/Weekly/Monthly report detected ({time_diff}), applying -1 hour shift (Querying 4:00 AM when user selects 5:00 AM)")
        else:
            # For hourly/multi-hour reports, use exact time (no shift)
            logger.info(f"📊 [MIL-A Summary] Hourly/Multi-hour report detected ({time_diff}), using exact time (no shift)")
        
        # ✅ Use times directly for querying, but add buffer to end time for hourly reports
        # For hourly reports, add 5 minute buffer to end time to catch records slightly after end time
        start_with_buffer = start_dubai
        if is_daily_or_longer:
            # Daily reports: Use exact end time
            end_with_buffer = end_dubai
        else:
            # Hourly/multi-hour reports: Add 5 minute buffer to catch records slightly after end time
            # Example: 11 PM to 12 PM should include records up to 12:05 PM
            end_with_buffer = end_dubai + timedelta(minutes=5)
            logger.info(f"📊 [MIL-A Summary] Hourly report - adding 5 minute buffer to query end time: {end_dubai} -> {end_with_buffer}")
        ...
        logger.info(f"  - start_with_buffer (Dubai): {start_with_buffer}")
        logger.info(f"  - end_with_buffer (Dubai): {end_with_buffer}")
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                # ✅ start_with_buffer and end_with_buffer are already naive datetimes in Dubai time
        ...
        if is_single_record:
            ...
            # For receiver weights, show the record's receiver values directly
            ...
            for rec in first_receivers:
                bin_id = rec.get("bin_id")
                mat_code = rec.get("material_code")
                mat_name = rec.get("material_name")
                weight = float(rec.get("weight_kg") or 0)
...
def get_scl_archive_summary():
    ...
    # Add buffer: +1 min for start (to exclude start hour exact), keep end as is
    # e.g. 8:00 to 9:00 request -> 8:01 to 9:00 range
    ...
    with get_db_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # ✅ Include optional order_name filter
            if order_name:
                cur.execute("""
                    SELECT *
...
def get_fcl_summary():
    ...
    # ✅ Parse timestamps from UTC ISO format
    import pytz
    start_parsed = datetime.fromisoformat(start_date.replace('Z', '+00:00'))
    end_parsed = datetime.fromisoformat(end_date.replace('Z', '+00:00'))
    ...
    # ✅ Add buffer: +1 min for start (to exclude start hour exact), but ONLY for daily reports (>= 24 hours)
    # For hourly/multi-hour reports (< 24 hours), include records from the start time
    # e.g. Daily: 5:00 to 5:00 next day (24 hours) -> 5:01 to 5:00 range (exclude exact 5:00 start)
    # e.g. Hourly/Multi-hour: 1:00 PM to 3:00 PM (2 hours) -> 1:00 PM to 3:00 PM range (include all records)
    time_diff = end_dubai - start_dubai
    is_daily_or_longer = time_diff >= timedelta(hours=23, minutes=59)  # >= 24 hours (daily/weekly/monthly)
    
    if is_daily_or_longer:
        # For daily/weekly/monthly reports (>= 24 hours), add +1 minute buffer to exclude exact start time
        # This prevents including records from the previous day's end time
        start_with_buffer = start_dubai + timedelta(minutes=1)
        logger.info(f"🕐 [FCL Summary] Daily/Weekly/Monthly report detected ({time_diff}), using +1 minute buffer")
    else:
        # For hourly/multi-hour reports (< 24 hours), include records from the start time (no buffer)
        start_with_buffer = start_dubai
        logger.info(f"🕐 [FCL Summary] Hourly/Multi-hour report detected ({time_diff}), using exact start time (no buffer)")
            ...
            for row in cursor.fetchall():
                ...
                if material_name:
                    ...
                    if bin_code == "21A":
                        ...
                        plc_bin_id = int(bin_code) if bin_code.isdigit() else bin_code
...
def get_ftra_archive_summary():
    """
    ✅ FTRA Summary Calculation (like SCL/FCL)
    Sums hourly totals across all records in the time range
    """
    from flask import request, jsonify
    from datetime import datetime, timedelta
